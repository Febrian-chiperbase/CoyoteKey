...
func main() {
	...
	setupAndRun(*targetURLFlag, *modeFlag, *threadsFlag, *proxyURLFlag, *timeoutFlag, *verboseFlag, bruteKeyArgs{
		...
		baseURL: *targetURLFlag, pathWordlistPath: *pathWordlistPath, paramWordlistPath: *paramWordlistPath,
		maxDepth: *maxDepthFlag, discoveryMethodsRaw: *discoveryMethodsRaw, fuzzMethodsRaw: *fuzzMethodsFlag,
...
func runDiscoveryMode(args discoveryArgs) {
	...
	for depth := 0; depth <= args.maxDepth; depth++ {
		...
		for _, basePath := range currentLevelPathsToExplore {
			// Untuk kedalaman > 0, kita hanya gunakan wordlist dari file.
			// Untuk kedalaman 0, kita gunakan gabungan dari spec dan file.
			currentWordlist := wordlistForThisDepth
			if depth > 0 {
				currentWordlist = pathWordlistFromFile
			}
			for _, segment := range currentWordlist {
				jobCount++
